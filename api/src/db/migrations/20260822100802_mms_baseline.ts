...
export class Migration20260822100802_mms_baseline extends Migration {
  ...
  override async up(): Promise<void> {
    ...
    this.addSql(`alter table "permissions" add constraint "permissions_code_unique" unique ("code");`);

    this.addSql(`create table "roles" ("id" uuid not null default gen_random_uuid(), "created_at" timestamptz not null, "updated_at" timestamptz not null, "code" text check ("code" in ('SUPERADMIN', 'SENSEI', 'MENTOR', 'MENTEE')) not null, "name" varchar(255) not null, constraint "roles_pkey" primary key ("id"));`);
    this.addSql(`alter table "roles" add constraint "roles_code_unique" unique ("code");`);

    this.addSql(`create table "roles_permissions" ("id" uuid not null default gen_random_uuid(), "created_at" timestamptz not null, "updated_at" timestamptz not null, "role_id" uuid not null, "permission_id" int not null, constraint "roles_permissions_pkey" primary key ("id"));`);
    this.addSql(`create index "roles_permissions_role_id_index" on "roles_permissions" ("role_id");`);
    this.addSql(`create index "roles_permissions_permission_id_index" on "roles_permissions" ("permission_id");`);
    ...
    this.addSql(`create table "sessions" ("id" uuid not null default gen_random_uuid(), "created_at" timestamptz not null, "updated_at" timestamptz not null, "token" varchar(255) not null, "user_id" uuid not null, "expires_at" timestamptz not null, "ip_address" varchar(255) null, "user_agent" text null, constraint "sessions_pkey" primary key ("id"));`);
    ...
    this.addSql(`alter table "accounts" add constraint "accounts_provider_id_account_id_unique" unique ("provider_id", "account_id");`);

    this.addSql(`create table "verifications" ("id" uuid not null default gen_random_uuid(), "created_at" timestamptz not null, "updated_at" timestamptz not null, "identifier" varchar(255) not null, "value" varchar(255) not null, "expires_at" timestamptz not null, constraint "verifications_pkey" primary key ("id"));`);
    this.addSql(`create index "verifications_identifier_index" on "verifications" ("identifier");`);

    this.addSql(`alter table "roles_permissions" add constraint "roles_permissions_role_id_foreign" foreign key ("role_id") references "roles" ("id") on update cascade;`);
    this.addSql(`alter table "roles_permissions" add constraint "roles_permissions_permission_id_foreign" foreign key ("permission_id") references "permissions" ("id") on update cascade;`);
