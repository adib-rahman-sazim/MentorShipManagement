...
export class Migration20260912204214_add_user_permission_overrides extends Migration {
  override async up(): Promise<void> {
    this.addSql(
      `create table "user_permission_overrides" ("id" uuid not null default gen_random_uuid(), "created_at" timestamptz not null, "updated_at" timestamptz not null, "user_id" uuid not null, "permission_id" int not null, "effect" text check ("effect" in ('ALLOW', 'REVOKE')) not null, "reason" text null, "deleted_at" timestamptz null, constraint "user_permission_overrides_pkey" primary key ("id"));`,
    );
    this.addSql(
