...
# Run 'pnpm run test:e2e' in the first pane
tmux send-keys -t $SESSION:Test-Lint "cd $WORKDIR && pnpm run test:e2e" C-m

# Create a horizontal split and run 'pnpm run lint' in the new pane
tmux split-window -h -t $SESSION:Test-Lint
tmux send-keys -t $SESSION:Test-Lint.1 "cd $WORKDIR && pnpm run lint" C-m

# Create a horizontal split and run 'pnpm run test' in the new pane
tmux split-window -h -t $SESSION:Test-Lint
tmux send-keys -t $SESSION:Test-Lint.2 "cd $WORKDIR && pnpm run test" C-m
