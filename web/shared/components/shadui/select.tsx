...
type TSelectLabelsContextValue = {
  labels: Map<unknown, React.ReactNode>;
  registerLabel: (value: unknown, label: React.ReactNode) => void;
};

const SelectLabelsContext = React.createContext<TSelectLabelsContextValue | null>(null);

function Select<Value, Multiple extends boolean | undefined = false>({
  children,
  ...props
}: SelectPrimitive.Root.Props<Value, Multiple>) {
  const [labels] = React.useState(() => new Map<unknown, React.ReactNode>());
    ...
    >
      {(value: unknown) => {
        if (value !== null && value !== undefined && value !== "") {
          ...
          return label ?? renderValue?.(value) ?? String(value);
        }
    ...
    <SelectPrimitive.Portal>
      ...
      >
        <SelectPrimitive.Popup
          ...
          className={cn(
            "bg-popover text-popover-foreground data-open:animate-in data-closed:animate-out data-closed:fade-out-0 data-open:fade-in-0 data-closed:zoom-out-95 data-open:zoom-in-95 data-[side=bottom]:slide-in-from-top-2 data-[side=left]:slide-in-from-right-2 data-[side=right]:slide-in-from-left-2 data-[side=top]:slide-in-from-bottom-2 ring-foreground/10 min-w-(--anchor-width) rounded-md shadow-md ring-1 duration-100 relative isolate z-50 max-h-(--available-height) origin-(--transform-origin) overflow-x-hidden overflow-y-auto",
            className,
          )}
          {...props}
        >
